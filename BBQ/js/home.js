...
// ฟังก์ชันโหลดข้อมูลสินค้าจาก API
async function loadProducts() {
    try {
        ...
        if (data.success && data.products) {
            ...
            // Render products หลังจากโหลดเสร็จ
            UIManager.renderProducts();
        } else {
            console.error('❌ Failed to load products:', data.message);
            // แสดงข้อความว่าไม่สามารถโหลดสินค้าได้
            $('#productlist').html(`
                <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #999;">
                    <i class="fas fa-exclamation-circle" style="font-size: 3rem; margin-bottom: 20px;"></i>
                    <p style="font-size: 1.2rem;">Unable to load products. Please refresh the page.</p>
                </div>
            `);
        }
    } catch (error) {
        ...
        $('#productlist').html(`
            <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #999;">
                <i class="fas fa-exclamation-circle" style="font-size: 3rem; margin-bottom: 20px;"></i>
                <p style="font-size: 1.2rem;">Network error. Please check your connection.</p>
            </div>
        `);
    }
}
        ...
        if (PRODUCTS.length === 0) {
            $('#productlist').html(`
                <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #999;">
                    ...
                    <p style="font-size: 1.2rem;">No products available at the moment.</p>
                </div>
            `);
            return;
        }
